...
async function main() {
  ...
  async function sendAndWait(text, cancelState = null) {
    ...
    // 清空并填入文本（直接用 evaluate 操作 DOM，完美保留 \n 换行）
    await editor.evaluate((el, text) => {
      const isRich = el.getAttribute('contenteditable') === 'true' || el.tagName.toLowerCase() === 'div';
      if (isRich) {
        // contenteditable 元素：用 textContent 保留换行，页面自动渲染 <br>
        el.textContent = text;
      } else {
        // 普通 input / textarea：用 value
        el.value = text;
      }
      // 派发事件，让 React/Vue 等框架感知到输入变化
      el.dispatchEvent(new Event('input', { bubbles: true }));
      el.dispatchEvent(new Event('change', { bubbles: true }));
    }, text);

    await page.waitForTimeout(300); // 等待页面状态更新
